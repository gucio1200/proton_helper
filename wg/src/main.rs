...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let web_app_version = "web-account@5.0.310.0";
    let mut headers = HeaderMap::new();
    headers.insert("x-pm-appversion", HeaderValue::from_str(web_app_version)?);
    ...
    while running.load(std::sync::atomic::Ordering::SeqCst) {
        ...
        servers.retain(|s| {
            ...
            status == 1
                && (!countries.contains(&entry_country) && !countries.contains(&exit_country))
                    == false
                && server_tier == tier
        ...
        if servers.is_empty() {
            println!("[WARN] No servers found for the selected criteria with Status=1.");
            thread::sleep(Duration::from_secs(5));
            continue;
        }

        // Shuffle and select one server
        ...
        println!("Endpoint: {}:51820", endpoint_ip);
        println!(
            "PublicKey: {}",
            reg["Features"]["peerPublicKey"].as_str().unwrap_or("")
        );
        println!("PrivateKey (x25519): {}", x25519_priv);
        println!("------------------------------------");
...
// Convert EC private key to x25519
...
// Update MikroTik WireGuard via SSH
fn update_mikrotik_wg(
    ...
    println!("[SSH] Authenticating as {}", user);
    ...
    for cmd in commands {
        ...
        println!("[SSH] Output:\n{}", s);
        channel.wait_close()?;
        println!("[SSH] Exit status: {}", channel.exit_status()?);
    }
